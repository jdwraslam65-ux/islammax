...
const glassStyle = {
  ...
  boxShadow: '0 24px 64px rgba(0,0,0,0.4), inset 0 1.5px 0 rgba(255,255,255,0.28), inset 0 -1px 0 rgba(255,255,255,0.06)',
};
...
function BranchCard({ branch, onClick, index }) {
  ...
  return (
    <motion.div onClick={onClick}
      ...
      <div className="absolute -inset-3 rounded-[2.5rem] opacity-0 group-hover:opacity-60 transition-opacity duration-500 blur-2xl pointer-events-none"
        style={{ background:`radial-gradient(circle, ${branch.glow}, transparent 70%)` }} />
      <div className="relative rounded-[2.5rem] p-10 text-center overflow-hidden" style={glassStyle}>
        ...
        <div className="absolute inset-0 rounded-[2.5rem] pointer-events-none opacity-20"
          style={{ background:`linear-gradient(135deg,${branch.orb[0]}33 0%,transparent 50%,${branch.orb[1]}22 100%)` }} />
        ...
        <p className="relative z-10 text-sm" style={{ color:'rgba(255,255,255,0.4)' }}>اضغط للبدء</p>
...
function ResultBadge({ result }) {
  ...
  const color = s >= 14 ? ['#10b981','#06b6d4'] : s >= 10 ? ['#3b82f6','#818cf8'] : ['#f43f5e','#fb923c'];
  ...
  return (
    <motion.div initial={{ opacity:0, scale:0.85, y:16 }} animate={{ opacity:1, scale:1, y:0 }}
      ...
      <div className="absolute inset-0 rounded-3xl pointer-events-none opacity-25"
        style={{ background:`radial-gradient(ellipse at 50% 110%,${color[0]}55,transparent 65%)` }} />
      <p className="text-xs font-semibold tracking-widest uppercase mb-2 relative z-10" style={{ color:'rgba(255,255,255,0.38)' }}>المعدل المتوقع</p>
      <div className="relative z-10 text-6xl font-black mb-3"
        style={{ background:`linear-gradient(135deg,${color[0]},${color[1]})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', filter:`drop-shadow(0 0 20px ${color[0]}88)` }}>
        {result}
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {!selectedBranch && (
            <motion.div key="select" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0, y:-20 }}>
              <div className="text-center mb-14">
                ...
                <p style={{ color:'rgba(255,255,255,0.38)' }}>لحساب معدل البكالوريا التقديري</p>
              </div>
          ...
          {selectedBranch && (
            <motion.div key="calc" initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} transition={{ duration:0.5 }}>
              <div className="relative rounded-[2.5rem] p-8 overflow-hidden" style={glassStyle}>
                <div className="absolute top-0 left-12 right-12 h-px" style={{ background:'linear-gradient(90deg,transparent,rgba(255,255,255,0.5),transparent)' }} />
                ...
                {/* Subjects */}
                <div className="relative z-10 space-y-3 max-h-[420px] overflow-y-auto pl-1" style={{ scrollbarWidth:'thin', scrollbarColor:'rgba(255,255,255,0.1) transparent' }}>
                  {Object.entries(coeffs[selectedBranch]).map(([sub, coef]) => (
                    <motion.div key={sub}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium" style={{ color:'rgba(255,255,255,0.85)' }}>{sub}</span>
                      </div>
                      <input type="number" min="0" max="20" step="0.25"
                        value={scores[sub] || ''}
                        onChange={e => setScores({ ...scores, [sub]: e.target.value })}
                        placeholder="—"
                        className="w-20 text-center font-bold text-white outline-none rounded-xl py-2 transition-all focus:scale-105 placeholder:text-white/20"
                        style={{ background:'rgba(0,0,0,0.25)', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'inset 0 1px 0 rgba(255,255,255,0.05)' }} />
                    </motion.div>
                  ))}
                </div>
